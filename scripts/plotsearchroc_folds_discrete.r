...
betagrasp <- read.table('d1ubia_.tsrchd.tt.rtab', header=TRUE)
immunoglobulin <- read.table('d1ae6h1.tsrchd.tt.rtab', header=TRUE)
timbarrel <- read.table('d1tima_.tsrchd.tt.rtab', header=TRUE)
plait <- read.table('d1bhne_.tsrchd.tt.rtab', header=TRUE)
greekkey <- read.table('d1h6rb_.tsrchd.tt.rtab', header=TRUE)
keybarrel <- read.table('d1tttb1.tsrchd.tt.rtab', header=TRUE)
jellyroll <- read.table('d2phlb1.tsrchd.tt.rtab', header=TRUE)
nadbinding <- read.table('d1f6dc_.tsrchd.tt.rtab', header=TRUE)


# EPS suitable for inserting into LaTeX
postscript('searchroc_folds_discrete.eps',onefile=FALSE,paper="special",horizontal=FALSE, 
           width = 9, height = 6)

#plot(c(0,1),c(0,1),type='l',xlim=c(0,1),ylim=c(0,1),xlab="False Positive Rate",ylab="True Positive Rate",lty=3,main='ROC curves (using distance information) against ASTRAL 95% seq id SCOP as truth')
