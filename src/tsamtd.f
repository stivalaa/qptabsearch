...
*
* $Id: tsamtd.F 3248 2010-01-21 01:27:42Z alexs $
*=======================================================================

      subroutine tsamtd(tab1,ldt1,n1,tab2,ldt2,n2,
     $     ltype,lorder,dmat1,dmat2,score,ssemap,info)

      implicit none
*
* tsamtd - find match score of two tableaux using simulated annealing
*
...
*     ..
*     .. Local Scalars ..
      integer k,iter,ssei,oldj,newj,newscore
      integer startj,maxscore,endj,restart
      logical lunmap
...
*           generate neighbour state by picking random SSE in tab1
*           and moving its mapping to a random SSE in tab2, mainting
*           constraints
            ssei = irrand(n1)
*           TODO for now always obeying type constraint (LTYPE)
 ...
 120              if (endj .eq. 0 .and. ssei + k .le. n1) then
            ...
            newj = randtypeind(tab2, ldt2, n2, startj, 
...
*              also try removing the SSE from matching
            ...
            newscore = tmscord(tab1, ldt1, n1, tab2, ldt2, n2, 
     $           dmat1, dmat2, ssemap)
...
*           end of while (iter .lt. maxiter) 
         ...
         restart = restart + 1
*        end of while (start .lt. maxstart)
         goto 10
      endif
      score = maxscore
