...
*
* $Id: thmatn.F 2969 2009-11-22 01:29:39Z astivala $
*=======================================================================

      subroutine thmatn(omega1,ldt1,n1,omega2,ldt2,n2,
     $     ltype,lorder,dmat1,dmat2,score,ssemap,info)

      implicit none
*
* thmatn - find match score of two tableaux using local search heuristic
*
...
*     ..
*     .. Array Arguments ..
      character*2   omega1(ldt1,*),omega2(ldt2,*)
...
*     .. External Subroutines and Functions ..
      ...
      integer  irrand
      external irrand
      external firsttypeindn
      integer  firsttypeindn
      external icopy
      external thinitn
*     ..
...
*        'hill climbing': find max score for sse in omega2 (db structure)
*        TODO at the moment, ONLY going forwards i.e. assuming LORDER
 ...
 105        if (endj .eq. 0 .and. ssei + k .le. n1) then
         ...
         newj = firsttypeindn(omega2, ldt2, n2, startj, 
...
C               write(*,*) 'x',ssei,maxj
 ...
 115           if (endj .eq. 0 .and. ssei + k .le. n1) then
            ...
            newj = firsttypeindn(omega2, ldt2, n2, newj+1, 
     $           omega1(ssei,ssei), revmap, endj)
C            write (*,*) (ssemap(k), k= 1, n1)
C            write (*,*) iter, ssei, startj, endj, newj
            goto 110
