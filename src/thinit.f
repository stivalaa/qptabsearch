...
*
* $Id: thinit.F 2969 2009-11-22 01:29:39Z astivala $
*=======================================================================

      subroutine thinit(tab1,ldt1,n1,tab2,ldt2,n2,
     $     ltype,lorder,lunmap,ssemap,revmap,info)

      implicit none
*
* thinit - initial mapping between two structrs for herurist methods
*
...
*        if true, the query struct (tab1) is allowed to have one or
*        more SSEs not mapped (0 in the ssemap). Otherwise, all of
*        its SSEs must be mapped and so tab1 must be smaller than tab2.
*        
...
*     TODO: maybe do something else if LORDER flag is not set
      if (ltype) then
         j = 1
         do 10 i = 1, n1
 5          if (j .lt. n2 .and. (tab1(i,i) .ne. tab2(j,j))) then
               j = j + 1
               goto 5
            endif
            if (j .ge. n2) then
...
*               then could assign in some other order
               if (.not. lunmap) then
*                 couldn't map all SSEs in tab1, but were required to
                  info = 1
               else
